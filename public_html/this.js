	...
	ThisApp.prototype = {
				...
				this.container
						...
						.on('click', '[this-goto]', function (e) {
							...
							if (__this.attr('this-page-title')) {
								var page = _this._('[this-type="pages"] [this-type="page"][this-id="'
										+ __this.attr('this-goto') + '"]'),
		...
		loadCollections: function () {
			var _this = this,
					...
					.each(function () {
						var content = _this._(this).html();
						_this._(this).children().attr('this-cache', '').hide();
		...
		loadModels: function () {
			var _this = this,
					...
					.each(function () {
						var __this = _this._(this),
								content = __this.html();
						__this.parent().append('<div this-cache style="display:none">' + content
								+ '</div>');
						if (__this.attr('this-collection')) {
							var model = _this.getModelFromCollectionStore(__this.attr('this-mid'),
									__this.attr('this-collection'));
							if (model) {
								--_this.models;
								...
								_this.loadData(this, model, content, false);
								_this.pageLoaded();
								return;
							}
						}
		 ...
		 * @returns ThisApp
		 */
		cache: function (type, id, data, update) { 			// retrieving
			if (!type)
				return this.store();
		...
		saveModelToCollectionStore: function (model, collection_id, uid) {
			return this.tryCatch(function () {
				var collection = this.cache('collection', collection_id),
						data = {data: {}};
				data.data[model[collection.uid || uid]] = model;
				this.cache('collection', collection_id, data, true);
				return this;
			});
		},
		/**
		 ...
		 * @returns ThisApp
		 */
		loadData: function (container, data, content, save) {
