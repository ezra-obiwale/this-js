...
console.error(message);return this;},toObject:function(string){return this.tryCatch(function(){return JSON.parse(string);});},toJSON:function(object){return this.tryCatch(function(){return JSON.stringify(object);});},createElement:function(str,tag){var d=document.createElement(tag||'div');if(!str)
...
this.container=this._('body');this.container.find('page,model,collection,layout,component,[this-type]').attr('this-loaded','');if(!this._('[this-type="pages"]').length){this._('body').append('<div this-type="pages" style="display:none"/>').find('[this-type="pages"]').append(this._('[this-type="page"]:not([this-loaded])').hide());}
...
return;e.preventDefault();var __this=_this._(this);var _model=__this.closest('model,[this-type="model"]'),_do=__this.closest('[this-do="delete"]'),__model=new Model(_model.attr('this-mid'),{},{app:_this,name:__this.attr('this-model')||_model.attr('this-id')||_do.attr('this-model'),uid:_model.attr('this-uid')||_do.attr('this-uid')||'id',url:__this.attr('this-delete')||_model.attr('this-url')||_do.attr('this-action'),method:'delete'});if(false===_this.__.callable(_this.beforeDelete).call(this,__model,_model.items[0])){_model.trigger('delete.canceled');return;}
...
id=_this.page.attr('this-mid');var _model=new Model(id,data,{app:_this,name:__this.attr('this-model')||_this.page.attr('this-model'),uid:__this.attr('this-uid')||__this.attr('this-model-uid')||_this.page.attr('this-model-uid'),url:__this.attr('this-action')||__this.attr('this-url')||_this.page.attr('this-action')||_this.page.attr('this-url'),method:method});_model.save(_model.url===null,function(data){var model=_this.config.dataKey?data[_this.config.dataKey]:data;if(model){if(creating){__this.items[0].reset();if(!__this.hasAttr('this-binding')&&!__this.closest('[this-binding]').length)
...
+'),filters.lcase("'+_search.val()+'"))';});if(_form.attr('this-reload')){_collection=page.find(internal.selector.call(_this,_form.attr('this-reload'),':not([this-cache])'));_collection.attr('this-filter',filter).attr('this-search',keys.join(','));page.attr('this-query',_search.val().trim());_this.loadCollection(_collection,replaceState);}
...
var data=internal.cache.call(this,type.toLowerCase());delete data[id];return this.store(type.toLowerCase(),data);},modelFromStore:function(model_id,model_name){return this.tryCatch(function(){var _collection=internal.cache.call(this,'model',model_name);return _collection.data[model_id];});},modelToStore:function(model_id,model,model_name,uid){return this.tryCatch(function(){var collection=internal.cache.call(this,'model',model_name)||{data:{},uid:uid,length:0};if(!collection.data[model_id||model[collection.uid||uid]])
collection.length++;collection.data[model_id||model[collection.uid||uid]]=model;internal.cache.call(this,'model',model_name,collection);return this;});},removeModelFromStore:function(model_id,collection_id){this.tryCatch(function(){var collection=internal.cache.call(this,'model',collection_id);delete collection.data[model_id];internal.cache.call(this,'model',collection_id,collection,true);return this;});},fullyFromURL:function(type,id,success,error){if(!this.config.paths){this.__.callable(error).call(this);return;}
var _this=this,pathConfig=this.config.paths[type+'s'],url=pathConfig.dir+id+pathConfig.ext;this.request(url,function(data){var elem=_this.__.createElement(data);if(!elem.length||elem.length>1||(!elem.is(type)&&elem.attr('this-type')!==type)){elem=_this._('<div this-type="'+type+'" this-id="'
+id+'" />').html(data);}
...
replaceInState=true;}
this.page=page.clone();internal.loadLayouts.call(this,replaceInState);}
...
+'"]:not([this-dead])').attr('this-current','').removeAttr('this-layout').show();if(this.page.attr('this-load-css')){var url=this.config.paths.css+this.page.attr('this-load-css').trim();if(!url.endsWith('.css'))
url=+'.css';if(this.config.debug)
url+='?'+Math.round(Math.random()*99999);var link=this.__.createElement(null,'link').attr('type','text/css').attr('rel','stylesheet').attr('href',url);this.page.prepend(link);}
if(this.page.attr('this-load-js')){var url=this.config.paths.js+this.page.attr('this-load-js');if(!url.endsWith('.js'))
url=+'.js';if(this.config.debug)
url+='?'+Math.round(Math.random()*99999);var script=this.__.createElement(null,'script').attr('type','application/javascript').attr('src',url);this.page.append(script);}
...
this.config.titleContainer.html(this.page.attr('this-title'));if(this.page.attr('this-url')){this.request(this.page.attr('this-url'),function(data){_this.page.html(data);internal.loadCollections.call(_this,replaceInState,true);},function(){},{},'text');}
else{internal.loadCollections.call(this,replaceInState,true);}}.bind(this));},doTar:function(__this,noShow){__this=this._(__this);var type=__this.attr('this-type')||__this.items[0].tagName.toLowerCase(),tar=type+'#'+__this.attr('this-id');if(this.tar[tar]){if(type==='page'&&this.tar[tar].reading){__this.attr('this-reading',true);delete this.tar[tar].reading;this.tar['model#'+__this.find('model,[this-type="model"]').attr('this-id')]=this.tar[tar];}
...
this.__.callable(callback).call(this);components.each(function(){var __this=_this._(this);if(__this.attr('this-url')){var url=__this.attr('this-url');if(_this.config.paths&&_this.config.paths.components)
url=_this.config.paths.components+url;_this.request(url,function(data){internal.loadComponent.call(_this,__this,_this._(data),callback);},function(){_this.__.callable(callback).call(_this);},{},'text');}
...
this.__.callable(callback).call(this);},loadCollections:function(replaceState,chain){var _this=this;var collections=this.page.find('collection:not([this-loaded])'
+':not([this-cache]),'
+'[this-type="collection"]:not([this-loaded]):not([this-cache])');this.__proto__.collections+=collections.length;if(chain&&!collections.length)
internal.loadModels.call(this,replaceState,chain);collections.each(function(){var __this=_this._(this).addClass('loading');if(!__this.attr('this-model'))
...
__this.removeClass('loading');internal.loadCollection.call(_this,__this,replaceState,true,chain);});return this;},loadCollection:function(__this,replaceState,looping,chain){__this=this._(__this).addClass('loading');var _this=this,ignore=_this.page.attr('this-ignore-cache')||'',content=__this.html(),cached=internal.cache.call(_this,'model',__this.attr('this-id')),model_name=__this.attr('this-model'),model_to_bind=_this.page.find('model[this-id="'+model_name
...
model_to_bind.attr('this-bind',true);if(_this.config.cacheData&&!_this.__.contains(ignore,'collection#'+__this.attr('this-id'))&&cached&&cached.length&&((cached.expires&&cached.expires<Date.now())||!cached.expires)){if(looping)
_this.__proto__.collections--;var data={};if(_this.config.dataKey)
data[_this.config.dataKey]=cached.data;else
data=cached.data;internal.loadData.call(this,__this,data,content,false,false);__this.attr('this-loaded','').trigger('collection.cache.loaded');if(chain&&!this.collections)
internal.loadModels.call(this,replaceState,chain);}
else if(__this.attr('this-url')){var data={},save=true;if(_this.page.attr('this-query')){data['query']=_this.page.attr('this-query');save=false;}
if(__this.attr('this-search')){data['keys']=__this.attr('this-search');}
_this.request(__this,function(data,uid){if(uid)
__this.attr('this-model-uid',uid);if(looping)
_this.__proto__.collections--;internal.loadData.call(_this,__this,data,content,false,save);__this.attr('this-loaded','').trigger('collection.loaded');if(chain&&!this.collections)
internal.loadModels.call(_this,replaceState,chain);},function(){if(looping)
_this.__proto__.collections--;if(chain&&!this.collections)
internal.loadModels.call(_this,replaceState,chain);},data);}
else{this.__proto__.collections--;if(chain&&!this.collections)
internal.loadModels.call(this,replaceState,chain);return;}},loadModels:function(replaceState,chain){var _this=this;if(this.components){setTimeout(function(){internal.loadModels.call(_this,replaceState);},300);return;}
...
internal.loadForms.call(this,null,null,replaceState,chain);models.each(function(){internal.loadModel.call(_this,this,null,replaceState,chain);});return this;},loadModel:function(target,data,replaceState,chain){var __this=this._(target),ignore=this.page.attr('this-ignore-cache')||'',content=__this.outerHtml(),_this=this,fromCache=false;__this=internal.doTar.call(this,__this,true);if(!data&&!__this.attr('this-url')){this.__proto__.models--;if(chain&&!this.models)
internal.loadForms.call(this,null,null,replaceState,chain);return;}
if(__this.siblings('[this-cache="'
+__this.attr('this-id')+'"]').length){content=__this.siblings('[this-cache="'+__this.attr('this-id')+'"]').html();}
else{var cache=_this._(content.replace(/\(\{/g,'__obrace2__').replace(/\}\)/g,'__cbrace2__')).attr('this-cache',__this.attr('this-id'));__this.parent().append(cache);}
if(!data&&!_this.config.cacheData&&!_this.__.contains(ignore,'model#'
+__this.attr('this-id'))){data=internal.modelFromStore.call(this,__this.attr('this-mid'),__this.attr('this-id'));if(data)
--this.__proto__.models;fromCache=true;}
if(data){if(this.config.dataKey){var _m={};_m[this.config.dataKey]=data;data=_m;}
internal.loadData.call(this,__this,data,content,true,false);if(fromCache)
__this.attr('this-loaded','').trigger('model.cache.loaded');if(chain&&!this.models)
internal.loadForms.call(this,null,null,replaceState,chain);return;}
this.request(__this,function(data){--_this.__proto__.models;internal.loadData.call(_this,__this,data,content,true,true);__this.attr('this-loaded','').trigger('model.loaded');if(chain&&!this.models)
internal.loadForms.call(_this,null,null,replaceState,chain);},function(){--_this.__proto__.models;if(chain&&!this.models)
internal.loadForms.call(_this,null,null,replaceState.chain);});},loadForms:function(forms,model,replaceState,chain){var _this=this,isPage=false;if(!forms){isPage=this.page.is('form');forms=isPage?this.page:this.page.find('form');}
...
return;var data=_this.__.extend({},model),keys=_this.__.contains(key,'.')?keys=key.split('.'):keys=[key];_this.__.forEach(keys,function(i,v){data=data[v];});if(__this.attr('type')==='radio'||__this.attr('type')==='checkbox'){if(__this.attr('value')==data||data==on)
...
__this.attr('value',data);});return this;},pageLoaded:function(replaceState){if(!this.__proto__.collections&&!this.__proto__.models&&!this.__proto__.components){var _this=this;this.container.find('[this-inline-code]').each(function(){var __this=_this._(this);__this.replaceWith(_this._('<code this-code />').html(__this.html()));});this.container.find('[this-block-code]').each(function(){var __this=_this._(this);__this.replaceWith(_this._('<pre />').html(__this.html())).innerWrap('<code this-code />');});this.container.find('[this-code]').each(function(){var __this=_this._(this),tags=internal.parseBrackets.call(_this,'<','>',__this.html()),content=__this.html();_this.__.forEach(tags,function(i,v){content=content.replace(v,'&lt;'+v.substr(1,v.length-2)+'&gt;');});content=content.replace(/\n/g,'<br/>').replace(/\s/g,'&nbsp;');__this.html(content).removeAttr('this-code');});this.container.html(internal.processExpressions.call(this,this.container.html()));delete this.firstPage;this.page.attr('this-loaded','');this.page=this.container.find('page[this-id="'+this.page.attr('this-id')
+'"]:not([this-dead]), [this-type="page"][this-id="'
+this.page.attr('this-id')+'"]:not([this-dead])');this.page.trigger('page.loaded');internal.resolveTargetOnPage.call(this);internal.saveState.call(this,replaceState);internal.updatePage.call(this);}
...
if(this.__.isArray(data)||isModel===false){var _this=this,filter=container.attr('this-filter'),level=0;switch(container.children().get(0).tagName.toLowerCase()){case"td":level=3;content=_this._('<table />').html(content).outerHtml();break;case"tr":level=2;content=_this._('<table />').html(content).outerHtml();break;}
this.__.forEach(data,function(index,model){if(save){_data.data[model[container.attr('this-model-uid')||'id']||index]=model;_data.length++;}
var _content=internal.inLoop.call(_this,{index:index,model:model},filter,content.replace(/{{_index}}/g,index));if(!_content)
return;_content=_this._(internal.processExpressions.call(_this,_content,{index:index,model:model}));internal.doLoad.call(_this,container,model,_content,variables,false,level);});if(save&&container.attr('this-type')==='collection'){_data.uid=container.attr('this-model-uid')||'id';_data.url=container.attr('this-url');}}
else if(data&&isModel){if(save!==false&&container.attr('this-type')==='model'&&container.attr('this-id')){save_as=container.attr('this-id');}
if(save)
_data.data[data[container.attr('this-uid')||'id']]=data;internal.doLoad.call(this,container,data,content,variables,true);}
container.removeAttr('this-filter').show();if(this.config.cacheData&&save!==false)
internal.cache.call(this,'model',save_as,_data,true);return this;},doLoad:function(container,data,content,variables,expressions,level)
{if(expressions)
content=internal.processExpressions.call(this,content,data);container=this._(container);var _temp=internal.parseData.call(this,data,content,variables),id=container.attr('this-model-uid')||container.attr('this-uid')||'id';while(level){_temp=_temp.children();level--;}
if(internal.is.call(this,'collection',container)){_temp.attr('this-mid',data[id]||'').attr('this-uid',id).attr('this-type','model').attr('this-in-collection','').attr('this-url',container.attr('this-url')
...
return;exps.push('({'+v.split('})')[0]+'})');});return exps;},processExpressions:function(content,current){var _this=this,exps=internal.getExpressions.call(this,content);this.__.forEach(exps,function(i,v){_this.tryCatch(function(){content=content.replace(v,eval(v.trim().substr(2,v.trim().length-4)));});});return content;},parseData:function(data,content,variables,forCollection){if(!variables)
...
__this.append(_content);});__this.removeAttr('this-each');});if(forCollection){content=internal.inLoop.call(this,{index:forCollection,model:data},'true',_temp.outerHtml());content=internal.processExpressions.call(this,content,{index:forCollection,model:data});}
...
content=internal.fillVariables.call(this,variables,data,content);_temp.replaceWith(content);if(custom)
...
return false;});return value;},fillVariables:function(variables,data,content){var _this=this;this.__.forEach(variables,function(i,v){var value=internal.getVariableValue.call(_this,v,data);content=content.replace(v,value||value==0?value:'');});return content;},getDeepValue:function(variable,data){var value=data,vars=__.isObject(variable,true)?variable:variable.split('.');__.forEach(vars,function(i,v){if(!value)
...
this.config.titleContainer.html(state.title);this.store('last_page',state.id);internal.updatePage.call(this);this.page.trigger('page.loaded');},updatePage:function(){var deleted=internal.cache.call(this,'deleted')||{},created=internal.cache.call(this,'created')||{},updated=internal.cache.call(this,'updated')||{},collection=internal.cache.call(this,'model')||{},_this=this,_collections=this.page.find('collection[this-loaded],[this-type="collection"][this-loaded]'),_models=this.page.find('model:not([this-cache]),[this-type="model"]:not([this-cache])'),touched={deleted:{},created:false,updated:false,back:false,cancel:false};if(_collections.length){this.__.forEach(created,function(model_name,arr){var _collection=_this.page.find('collection[this-model="'+model_name
+'"][this-loaded],[this-type="collection"][this-model="'
+model_name+'"][this-loaded]');if(!_collection.length)
return;var __collection=collection[model_name],uid=__collection.uid;_this.__.forEach(arr,function(i,v){var tmpl=internal.parseData.call(_this,__collection.data[v],_collection.children('[this-cache]').clone().removeAttr('this-cache').outerHtml(),null,v),action=_collection.attr('this-prepend-new')?'prepend':'append';_collection[action](tmpl.attr('this-mid',v).attr('this-uid',uid).attr('this-type','model').attr('this-id',model_name).attr('this-url',_collection.attr('this-url')+v).attr('this-in-collection','').outerHtml());_this.__.arrayRemove(arr,i);});if(!created[model_name].length){delete created[model_name];internal.cleanupUpdates.call(_this,model_name,[created,updated,deleted]);}
...
return;_this.__.forEach(arr,function(id,v){_model.filter(function(){return this.getAttribute('this-mid')===id;}).each(function(){var __model=_this._(this),tmpl=internal.parseData.call(_this,v,__model.siblings('[this-cache]').clone().removeAttr('this-cache').outerHtml(),null,id);if(__model.hasAttr('this-in-collection'))
in_collection=true;__model.html(tmpl.html()).show();});});if(in_collection){delete updated[model_name];internal.cleanupUpdates.call(_this,model_name,[created,updated,deleted]);}
touched.updated=true;});this.__.forEach(deleted,function(model_name,arr){_this.__.forEach(arr,function(i,mid){var _model=_this.page.find('model[this-id="'+model_name
...
this.store('created',created);var del=false;this.__.forEach(touched.deleted,function(mod,arr){_this.__.forEach(arr,function(i,v){_this.__.arrayRemoveValue(deleted[mod],v);del=true;});if(!deleted[mod].length){delete deleted[mod];internal.cleanupUpdates.call(_this,mod,[created,updated,deleted]);}});if(del)
this.store('deleted',deleted);internal.loadForms.call(this);return touched.back?this.back():internal.saveState.call(this,true);},is:function(type,container){return this._(container).attr('this-type')===type||this._(container).is(type);},selector:function(id,append){id=id.split('#');if(!append)
append='';var attrs=id[id.length-1].split('[');id[id.length-1]=attrs[0];this.__.arrayRemove(attrs,0);if(attrs.length)
append+='['+attrs.join('[');return id.length>1?'[this-type="'+id[0]+'"][this-id="'+id[1]+'"]'+append+','+id[0]+'[this-id="'+id[1]+'"]'+append:'[this-id="'+id[0]+'"]'+append;},cleanupUpdates:function(model_name,updates){if(this.config.cacheData)
return;var canDelete=true;this.__.forEach(updates,function(){if(this[model_name])
canDelete=false;return canDelete;});if(canDelete)
internal.clearCache.call(this,'model',model_name);}}),Transitions=Object.create({show:function(pageOff,pageOn){pageOff.remove();pageOn.show();return this;}}),Filters=Object.create({lowercase:function(item){var _this=this;if(__.isArray(item)){var arr=[];__.forEach(item,function(i,v){var _item=_this.lcase(v);if(_item)
...
return str;return str.join(separator);},replace:function(str,options){return this.__(str,options,function(str,search,replace){return str.replace(new RegExp(eval(search)),replace);},function(str,search,replace){return str.replace(search,replace);});},__:function(str,options,funcA,funcB){if(__.isArray(str)){var arr=[],_this=this;__.forEach(str,function(i,v){arr.push(_this.__(v,options,funcA,funcB));});return arr;}
...
var _Model=Object.create({name:props&&props.name?props.name:null,app:props&&props.app?props.app:null,uid:props&&props.uid?props.uid:'id',url:props&&props.url?props.url:null,id:id,collection:props&&props.collection?props.collection:null,method:props&&props.method?props.method:null,attributes:__.isObject(attributes)?attributes:{},init:function(attr){this['get'+Filters.snakeToCamel(attr,true)]=function(){return this.attributes[attr];};this['set'+Filters.snakeToCamel(attr,true)]=function(val){this.attributes[attr]=val;return this;};},toURL:function(){return toURL(this.attributes);},save:function(cacheOnly,success,error,complete){var _this=this,method=this.id?'PUT':'POST';if(!this.app){console.error('Invalid model object');return false;}
else if(!this.name){_this.app.error('Cannot save an unnamed model.');return false;}
if(this.method)
method=this.method;if(cacheOnly)
internal.modelToStore.call(this.app,this.id,this.attributes,this.name,this.uid);else if(_this.url){var _success=function(data,id){var model=_this.app.config.dataKey?data[_this.app.config.dataKey]:data;if(model){internal.modelToStore.call(_this.app,_this.id,model,_this.name,_this.uid);var _action=_this.id?'updated':'created',action=_this.app.store(_action)||{};if(_this.id){if(!action[_this.name])
action[_this.name]={};action[_this.name][_this.id]=model;_this.app.store(_action,action);if(_this.collection)
_this.collection.models[_this.id]=model;}
else{_this.id=id||model[_this.uid];_this.url+=_this.id;if(!action[_this.name])
...
_this.attributes=model;internal.updatePage.call(_this.app);if(_this.collection){_this.collection.models[_this.id]=model;_this.collection.length++;}}
_this.app.__.callable(success).call(this,data);_this.app.__.callable(complete).call(this);},_error=function(e){_this.app.__.callable(error).call(this,e);_this.app.__.callable(complete).call(this,e);};if(this.app.dataTransport)
this.__.callable(this.app.dataTransport).call(this,{url:this.url,id:this.id,data:this.attributes,action:this.id?'update':'create',success:_success,error:_error});else
this.app.__.ajax({type:method,url:_this.app.config.baseURL?_this.app.config.baseURL+
...
return false;},remove:function(cacheOnly,success,error,complete){var _this=this;if(!this.app){console.error('Invalid model object');return false;}
else if(!this.name){_this.app.error('Cannot delete an unnamed model.');return false;}
if(cacheOnly)
internal.removeModelFromStore.call(this.app,this.id,this.name);else if(this.url){var _success=function(data,id){var model=_this.app.config.dataKey?data[_this.app.config.dataKey]:data;if(model){_this.id=id||model[_this.uid];_this.attributes=model;internal.removeModelFromStore.call(_this.app,_this.id,_this.name);var deleted=_this.app.store('deleted')||{};if(!deleted[_this.name])
deleted[_this.name]=[];deleted[_this.name]=_this.app.__.arrayRemoveValue(deleted[_this.name],_this.id,true);deleted[_this.name].push(_this.id);_this.app.store('deleted',deleted);if(_this.collection){delete _this.collection.models[_this.id];_this.collection.length--;}
internal.updatePage.call(_this.app);}
_this.app.__.callable(success).call(this,data);_this.app.__.callable(complete).call(this);},_error=function(e){_this.app.__.callable(error).call(this,e);_this.app.__.callable(complete).call(this,e);};if(this.app.dataTransport)
this.__.callable(this.app.dataTransport).call(this,{url:this.url,id:this.id,action:'delete',success:_success,error:_error});else
this.app.__.ajax({type:'delete',url:_this.app.config.baseURL+_this.url,success:_success,error:_error});}
...
return new ThisApp(config);this.version='1.0';if(config&&__.isObject(config)){this.config=this.__.extend(this.config,config);}};ThisApp.Transitions={add:function(name,func){if(name&&func&&!Transitions[name]){Transitions[name]=func;}
...
ThisApp.prototype[i]=v;});};ThisApp.prototype={events:[],config:{startWith:null,baseURL:location.origin+location.pathname,debug:false,dataKey:'data',titleContainer:null,cacheData:true,transition:'show',transitionOptions:{},layout:null,paths:{pages:{dir:'./pages',ext:'.html'},layouts:{dir:'./layouts',ext:'.html'},components:{dir:'./components',ext:'.html'},js:'./assets/js',css:'./assets/css'}},tar:{},components:0,collections:0,models:0,__:__,_:function(selector,debug){return _(selector,debug||this.config.debug);},setBaseUrl:function(url){this.config.baseURL=url;return this;},cacheData:function(status){if(!this.running)
this.config.cacheData=status;return this;},debug:function(debug){this.config.debug=debug!==false?true:false;this.__.__proto__.debug=this.config.debug;return this;},setDataKey:function(key){this.config.dataKey=key;return this;},setTitleContainer:function(container){this.config.titleContainer=this.running?this._(container):container;return this;},setTransition:function(transition,options){this.config.transition=transition;this.config.transitionOptions=options;return this;},getAvailableTransitions:function(){return Object.keys(Transitions);},getAvailableFilters:function(){return Object.keys(filters);},header:function(component_id){this.header=this._('component[this-id="'+component_id+'"], '+'[this-type="components"] [this-id="'+component_id+'"]');return this;},footer:function(component_id){this.footer=this._('component[this-id="'+component_id+'"],'
+'[this-type="components"] [this-id="'+component_id+'"]');return this;},setLayout:function(layout){this.config.layout=layout;return this;},setPagesPath:function(path,ext){if(!this.config.paths)
this.config.paths={};this.__proto__.config.paths.pages={dir:path,ext:ext||'.html'};return this;},setLayoutsPath:function(path,ext){if(!this.config.paths)
this.config.paths={};this.__proto__.config.paths.layouts={dir:path,ext:ext||'.html'};return this;},setComponentsPath:function(path,ext){if(!this.config.paths)
this.config.paths={};this.__proto__.config.paths.components={dir:path,ext:ext||'.html'};return this;},setJSPath:function(path){if(!this.config.paths)
this.config.paths={};this.__proto__.config.paths.js=path;return this;},setCSSPath:function(path){if(!this.config.paths)
this.config.paths={};this.__proto__.config.paths.css=path;return this;},start:function(page,fromState){if(this.running)
return this;if(page)
...
elem.trigger('loading.url');url=elem.attr('this-url');var isCollection=internal.is.call(this,'collection',elem);if(isCollection&&this.watchCallback){var model_name=elem.attr('this-model')||elem.attr('this-id');this.__.callable(this.watchCallback)(url,function(data,uid,event){if(!elem.hasAttr('this-loaded'))
return;var action=this.store(event)||{},id=data[uid];switch(event){case'created':internal.modelToStore.call(this,id,data,model_name,elem.attr('this-uid'));if(!action[model_name])
action[model_name]=[];action[model_name]=this.__.arrayRemoveValue(action[model_name],id,true);action[model_name].push(id);break;case'updated':internal.modelToStore.call(this,id,data,model_name,elem.attr('this-uid'));if(!action[model_name])
action[model_name]={};action[model_name][id]=data;break;case'deleted':internal.removeModelFromStore.call(this,id,model_name);if(!action[model_name])
action[model_name]=[];action[model_name]=this.__.arrayRemoveValue(action[model_name],id,true);action[model_name].push(id);break;}
this.store(event,action);internal.updatePage.call(this);elem.trigger('load.content.success');elem.trigger('load.content.complete');}.bind(this),function(message){this.error(message);elem.trigger('load.content.error');elem.trigger('load.content.complete');}.bind(this));}}
var _success=function(){_this.__.callable(success).apply(elem||this,Array.from(arguments));if(elem){elem.trigger('load.content.success');elem.trigger('load.content.complete');}},_error=function(){_this.__.callable(error).apply(elem||this,Array.from(arguments));if(elem){elem.trigger('load.content.error');elem.trigger('load.content.complete');}};if(elem&&this.dataTransport){this.__.callable(this.dataTransport).call(this,{action:'read',collection:isCollection,id:elem.attr('this-mid'),url:url,success:_success.bind(elem),error:_error.bind(elem)});return this;}
if(!url.startsWith('./')&&!url.startsWith('../')&&url.indexOf('://')===-1)
url=this.config.baseURL+url;return this.__.ajax({type:'get',url:url,dataType:responseType||'json',data:data||{},clearCache:this.config.debug,success:_success,error:_error});},watch:function(callback){this.watchCallback=callback;return this;},error:function(msg){if(!this.config.debug)
...
localStorage.setItem(key,value);return this;},tryCatch:function(callback,args){return this.__.tryCatch((this.__.callable(callback)).bind(this),args);},reload:function(){location.reload();return this;},collection:function(model_name){if(!model_name)
return new Collection([],{app:this});var collection=internal.cache.call(this,'model',model_name);return new Collection(collection.data,{name:model_name,app:this,uid:collection.uid||'id',url:collection.url||null,length:collection.length||0});},setDataTransport:function(callback){if(!callback)
return this;this.dataTransport=callback;return this;}};})();