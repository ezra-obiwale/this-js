...
if(!ignoreCache)
model=this.collection(model_name).model(_model.attr('mid'));if(model){model.bind(_target);_target.trigger('variables.binded',{data:model});}
...
return this;},doTar:function(__this,noShow){if(__this.attr('this-tar')){var tar=__this.attr('this-tar').split(';');this.__.forEach(tar,function(i,v){var split=v.split(':');if(split.length<2)
...
+':not([this-cache]),'
+'[this-type="collection"]:not([this-loaded]):not([this-cache])');this.__proto__.collections+=collections.length;collections.each(function(){var __this=_this._(this).addClass('loading');if(__this.children().length>1)
__this.innerWrap('<div/>');var content=__this.html(),cached=internal.cache.call(_this,'model',__this.attr('this-id')),model_name=__this.attr('this-model');if(!_this._('collection[this-id="'+__this.attr('this-id')+'"]:not(.loading),[this-type="collection"][this-id="'
...
__this.removeClass('loading');__this.html(__this.children().attr('this-cache',__this.attr('this-model')||'').hide().outerHtml().replace(/\(\{/g,'__obrace2__').replace(/\}\)/g,'__cbrace2__'));if(model_name&&_this.page.find('model[this-id="'+model_name
...
_this.request(this,function(data){_this.__proto__.collections--;internal.loadData.call(_this,this,data,content,false,save);__this.attr('this-loaded','').trigger('collection.loaded');internal.pageLoaded.call(_this,replaceState);},function(){},data);}});return this;},loadModel:function(target,data,replaceState){var __this=this._(target),ignore=this.page.attr('this-ignore-cache')||'',content=__this.outerHtml(),_this=this;if(!data&&!__this.attr('this-url')){this.__proto__.models--;return;}
...
+'" style="display:none">'
+content.replace(/\(\{/g,'__obrace2__').replace(/\}\)/g,'__cbrace2__')
+'</div>');if(!data&&!_this.__.contains(ignore,'model#'+__this.attr('this-id'))){data=internal.modelFromStore.call(this,__this.attr('this-mid'),__this.attr('this-id'));if(data)
...
+'"]:not([this-dead])').attr('this-current','').removeAttr('this-layout').show();var transit=this.__.callable(this.config.transition,true),wait;if(transit)
...
+this.page.attr('this-id')+'"][this-loaded]:not([this-dead])');this.page.trigger('page.loaded');internal.saveState.call(this,replaceState);}
...
return;_content=internal.processExpressions.call(_this,_content,{index:index,model:model});internal.doLoad.call(_this,container,model,_content,variables,false);});if(container.attr('this-type')==='collection'){_data.uid=container.attr('this-model-uid')||'id';_data.url=container.attr('this-url');}}
else if(data&&model){if(save!==false&&container.attr('this-type')==='model'&&container.attr('this-id')){save_as=container.attr('this-id');}
if(save)
...
internal.cache.call(this,'model',save_as,_data,true);return this;},doLoad:function(container,data,content,variables,expressions){if(expressions)
content=internal.processExpressions.call(this,content,data);container=this._(container);var _temp=internal.parseData.call(this,data,content,variables),id=container.attr('this-model-uid')||'id';if(internal.is.call(this,'collection',container)){_temp.attr('this-mid',data[id]||'').attr('this-uid',id).attr('this-type','model').attr('this-in-collection','').attr('this-url',container.attr('this-url')
...
variables=internal.parseBrackets.call(this,'{{','}}',content);if(this.__.isString(content))
content=content.replace(/__obrace2__/g,'({').replace(/__cbrace2__/g,'})');var _temp=this.__.isObject(content)?content:this._('<div/>').html(content),_this=this;_temp.find('[this-each]').each(function(){var __this=_this._(this),_data=data[__this.attr('this-each')],filter=__this.attr('this-filter'),each=__this.attr('this-each').trim(),content=__this.removeAttr('this-muted').html().replace(/__obrace__/g,'{{').replace(/__cbrace__/g,'}}').replace(/__obrace2__/g,'({').replace(/__cbrace2__/g,'})');__this.html('');if(!_data){if(each.startsWith('{(')){_data=internal.eval.call(_this,each.substr(1,each.length-2),data);}
...
return;_content=internal.processExpressions.call(_this,_content,__data);var _variables=internal.parseBrackets.call(_this,'{{','}}',_content);__this.append(internal.fillVariables.call(_this,_variables,__data,_content).replace(/{{key}}/g,key));});__this.removeAttr('this-each');});content=internal.inLoop.call(this,data,'true',_temp.html());content=internal.processExpressions.call(this,content,data);_temp.html(internal.fillVariables.call(this,variables,data,content));var children=_temp.children();if(children.length===1)
...
+'"][this-loaded]:not([this-dead])');if(this.config.titleContainer)
...
return true;},bind:function(elem){elem=this.app._(elem);elem.attr('this-mid',this.attributes[this.uid]).attr('this-uid',this.uid).attr('this-url',this.url);internal.loadModel.call(this.app,elem,this.attributes,true);internal.loadForms.call(this.app,elem.is('form')?elem:elem.find('form'),this.attributes);return this;}});if(__.isObject(attributes)){__.forEach(attributes,function(key){_Model.init(key);});}
