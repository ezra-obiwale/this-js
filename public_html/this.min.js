// ThisJS version 1.0.0 (http://thisjs.com) | (c) 2016 Ezra Obiwale | License (http://thisjs.com/#license)
...
result=Array.from(this.querySelectorAll(selector));});return _(result,this.debug);},filter:function(func){var _this=this,filtered=this.items.filter(function(v,i){func=_this.callable(func);return func?func.call(v,i,v):true;});return _(filtered,this.debug);},closest:function(selector){return this.tryCatch(function(){return _(this.items.length?this.items[0].closest(selector):[],this.debug);});},parent:function(){var parents=[];this.each(function(){parents.push(this.parentElement);});return _(parents,this.debug);},get:function(index){return this.items[index];},replaceWith:function(content){var replaced=[];this.each(function(){var _content=content.items[0];if(!_content)
...
this.__proto__.debug=debug!==false?true:false;this.length=this.items.length;return this;},internal=Object.create({call:function(_this,method,param){var args=Array.from(arguments);__.arrayRemove(args,0);__.arrayRemove(args,0);return this[method].apply(_this,args);},setup:function(){this.tryCatch(function(){var _this=this;this.__.__proto__.debug=this.config.debug;this.__.forEach(this.config.paths,function(i,v){if(!v.endsWith('/')&&i!=='ext')
_this.config.paths[i]=v+'/';});if(!this.notFound)
this.notFound=function(){if(_this.firstPage){if(_this.store('last_page')){_this.store('last_page',null);_this.loadPage(_this.store('last_page'));}
...
return;_this.page.trigger('page.leave');var go=__this.attr('this-goto').split('#'),goto=go[0],page=_this._('page[this-id="'+goto
+'"]:not([this-current]):not([this-dead]),[this-type="pages"]'
...
page.attr('this-tar',tar);this.pageTarget=go[1];_this.loadPage(goto);e.stop=true;}).on('click','[this-delete]',function(e){if(e.stop)
...
internal.restoreState.call(_this,e.state);});this.__.forEach(this.events,function(){_this.container.on(this.event,this.selector,this.callback);});});this.__proto__.running=true;return this;},bindToModel:function(_target,_model,ignoreCache){this.binding=true;var _this=this,model_name=_model.is('model')||_model.attr('this-type')==='model'?_model.attr('this-id'):_model.attr('this-model'),model;if(!_model.attr('this-mid')){this.error('Model to bind to does not have an id');return;}
...
+layout+'"]').clone();if(!_layout.length){internal.fullyFromURL.call(this,'layout',layout,function(_layout){_layout.find('[this-content]').html(_this.page);if(_layout.attr('this-extends'))
...
+__layout.attr('this-extends')+'"],'
+'[this-type="layouts"] [this-type="layout"][this-id="'
+__layout.attr('this-extends')+'"]').clone();if(!_layout.length){internal.fullyFromURL.call(this,'layout',__layout.attr('this-extends'),function(_layout){_layout.find('[this-content]').html(__layout);if(_layout.attr('this-extends'))
internal.extendLayout.call(_this,_layout,replaceInState);else
...
else{if(_layout.attr('this-url')){this.request(_layout.attr('this-url'),function(data){_layout.attr('this-id',__layout.attr('this-id')).removeAttr('this-url').html(data).find('[this-content]').removeAttr('this-content').html(__layout.removeAttr('this-id').removeAttr('this-type').show());if(_layout.attr('this-extends')){internal.extendLayout.call(_this,_layout,replaceInState);}
else{internal.finalizePageLoad.call(_this,_layout,replaceInState);}},function(){},{},'text');}
else{_layout.attr('this-id',__layout.attr('this-id')).find('[this-content]').removeAttr('this-content').html(__layout.removeAttr('this-id').removeAttr('this-type').show());if(_layout.attr('this-extends')){internal.extendLayout.call(this,_layout,replaceInState);}
...
else{internal.loadComponents.call(this);internal.loadCollections.call(this,replaceInState);internal.loadModels.call(this,replaceInState);internal.loadForms.call(this);this.page.attr('this-loaded','');internal.pageLoaded.call(this,replaceInState);}},pageLoaded:function(replaceState){if(!this.__proto__.collections&&!this.__proto__.models&&!this.__proto__.components){this.container.find('[this-inline-code]').innerWrap('<code/>').removeAttr('this-inline-code');this.container.find('[this-block-code]').innerWrap('<pre/>').removeAttr('this-block-code').children('pre').innerWrap('<code />');delete this.firstPage;this.page.trigger('page.loaded');internal.saveState.call(this,replaceState);}
...
+'[this-type="components"] [this-id="'+component_id+'"]');return this;},loadFromState:function(load){this.config.loadFromState=load!==undefined?load:true;return this;},setLayout:function(layout){this.config.layout=layout;return this;},setPathsExtension:function(ext){if(!ext.startsWith('.'))
ext='.'+ext;this.config.paths.ext=ext;return this;},setPagesPath:function(path){this.__proto__.config.paths.pages=path;return this;},setLayoutsPath:function(path){this.__proto__.config.paths.layouts=path;return this;},setComponentsPath:function(path){this.__proto__.config.paths.components=path;return this;},start:function(page){if(this.running)
return this;if(page)
this.config.startWith=page;this.container=this._(this.config.container).html('');this.firstPage=true;internal.setup.call(this);var target_page=location.hash.substr(1);if(this.config.loadFromState&&history.state&&target_page===this.store('last_page')){internal.restoreState.call(this,history.state);}
...
if(!newPage.length){if(this.config.paths.pages){var _this=this;internal.fullyFromURL.call(this,'page',page,function(elem){internal.pageFound.call(_this,elem,replaceInState);},function(){internal.notAPage.call(_this,page);});return this;}
