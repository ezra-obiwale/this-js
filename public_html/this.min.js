...
var _this=this,url=this.config.paths[type+'s']+id+this.config.paths.ext;this.request(url,function(data){var elem=_this.__.createElement(data);if(!elem.length||elem.length>1||((!elem.is(type)||elem.attr('this-type')!==type)&&elem.attr('this-id')!==id)){elem=_this._('<div this-type="'+type+'" this-id="'
+id+'" />').html(data);}
...
+'"]:not([this-dead])').attr('this-current','').removeAttr('this-layout').show();internal.loadComponents.call(this,function(){this.page=internal.doTar.call(this,this.page);var transit=this.__.callable(this.config.transition,true),wait;if(transit)
...
internal.loadModels.call(this,replaceState,chain);collections.each(function(){var __this=_this._(this).addClass('loading');if(__this.children().length>1)
...
model_to_bind.attr('this-bind',true);if(!_this.__.contains(ignore,'collection#'+__this.attr('this-id'))&&cached&&cached.length&&((cached.expires&&cached.expires<Date.now())||!cached.expires)){if(looping)
...
_this.request(__this,function(data){if(looping)
...
internal.loadData.call(this,__this,data,content,true,false);__this.attr('this-loaded','').trigger('model.cache.loaded');if(chain&&!this.models)
internal.loadForms.call(this,null,null,replaceState,chain);return;}
...
+this.page.attr('this-id')+'"]:not([this-dead])');this.page.trigger('page.loaded');internal.resolveTargetOnPage.call(this);internal.saveState.call(this,replaceState);}
...
return;_content=_this._(internal.processExpressions.call(_this,_content,{index:index,model:model}));internal.doLoad.call(_this,container,model,_content,variables,false,level);});if(container.attr('this-type')==='collection'){_data.uid=container.attr('this-model-uid')||'id';_data.url=container.attr('this-url');}}
...
{if(expressions)
content=internal.processExpressions.call(this,content,data);container=this._(container);var _temp=internal.parseData.call(this,data,content,variables),id=container.attr('this-model-uid')||'id';while(level){_temp=_temp.children();level--;}
...
return;exps.push('({'+v.split('})')[0]+'})');});return exps;},processExpressions:function(content,current){var _this=this,exps=internal.getExpressions.call(this,content);this.__.forEach(exps,function(i,v){_this.tryCatch(function(){content=content.replace(v,eval(v.trim().substr(2,v.trim().length-4)));});});return content;},parseData:function(data,content,variables){if(!variables)
...
__this.append(_content);});__this.removeAttr('this-each');});content=internal.inLoop.call(this,data,'true',_temp.outerHtml());content=internal.processExpressions.call(this,content,data);content=internal.fillVariables.call(this,variables,data,content);_temp.replaceWith(content);if(custom)
...
return false;value=value[v];});return value||'';},saveState:function(replace){var action='pushState';if(replace||this.firstPage)
...
this.config.titleContainer.html(state.title);this.store('last_page',state.id);internal.updatePage.call(this);this.page.trigger('page.loaded');},updatePage:function(){var deleted=internal.cache.call(this,'deleted')||{},created=internal.cache.call(this,'created')||{},updated=internal.cache.call(this,'updated')||{},collection=internal.cache.call(this,'model')||{},_this=this,_collections=this.page.find('collection,[this-type="collection"]'),_models=this.page.find('model:not([this-cache]),[this-type="model"]:not([this-cache])'),touched={deleted:{},created:false,updated:false,back:false,cancel:false};if(_collections.length){this.__.forEach(created,function(model_name,arr){var _collection=_this.page.find('collection[this-model="'+model_name
+'"],[this-type="collection"][this-model="'+model_name+'"]');if(!_collection.length)
...
return;_this.__.forEach(arr,function(id,v){_model.filter(function(){return this.getAttribute('this-mid')===id;}).each(function(){var __model=_this._(this),tmpl=internal.parseData.call(_this,v,__model.siblings('[this-cache]').clone().removeAttr('this-cache').outerHtml());if(__model.hasAttr('this-in-collection'))
in_collection=true;__model.html(tmpl.html()).show();});});if(in_collection)
...
ThisApp.prototype[i]=v;});};ThisApp.prototype={events:[],config:{startWith:null,baseURL:location.origin+location.pathname,debug:false,dataKey:'data',titleContainer:null,cacheData:true,transition:'show',transitionOptions:{},layout:null,paths:{pages:'./pages',layouts:'./layouts',components:'./components',ext:'.html'}},tar:{},components:0,collections:0,models:0,__:__,_:function(selector,debug){return _(selector,debug||this.config.debug);},setBaseUrl:function(url){this.config.baseURL=url;return this;},cacheData:function(status){if(!this.running)
...
+'[this-type="components"] [this-id="'+component_id+'"]');return this;},setLayout:function(layout){this.config.layout=layout;return this;},setPathsExtension:function(ext){if(!this.config.paths)
...
this.config.paths={};this.__proto__.config.paths.components=path;return this;},start:function(page,fromState){if(this.running)
return this;if(page)
...
elem.trigger('loading.url');url=elem.attr('this-url');if(internal.is.call(this,'collection',elem)&&this.watchCallback){var model_name=elem.attr('this-model')||elem.attr('this-id');this.__.callable(this.watchCallback)(url,function(data,id,event){var action=_this.store(event)||{};switch(event){case'created':if(_this.config.cacheData)
internal.modelToStore.call(_this,id,data,model_name,elem.attr('this-uid'));if(!action[model_name])
action[model_name]=[];action[model_name]=_this.__.arrayRemoveValue(action[model_name],id,true);action[model_name].push(id);break;case'updated':if(_this.config.cacheData)
internal.modelToStore.call(_this,id,data,model_name,elem.attr('this-uid'));if(!action[model_name])
action[model_name]={};action[model_name][id]=data;break;case'deleted':if(_this.config.cacheData)
internal.removeModelFromStore.call(_this,id,model_name);var action=_this.store('deleted')||{};if(!action[model_name])
action[model_name]=[];action[model_name]=_this.__.arrayRemoveValue(action[model_name],id,true);action[model_name].push(id);break;}
_this.store(event,action);internal.updatePage.call(_this);elem.trigger('load.content.success');elem.trigger('load.content.complete');},function(message){elem.trigger('load.content.error');elem.trigger('load.content.complete');});return this;}}
var _success=function(data){_this.__.callable(success).call(elem||this,data);if(elem){elem.trigger('load.content.success');elem.trigger('load.content.complete');}},_error=function(){_this.__.callable(error).call(elem||this,data);if(elem){elem.trigger('load.content.error');elem.trigger('load.content.complete');}};if(elem&&this.dataTransport){this.__.callable(this.dataTransport).call(this,{action:'read',id:elem.attr('this-mid'),url:url,success:_success.bind(elem),error:_error.bind(elem)});return this;}
